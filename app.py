...
# ══════════════════════════════════════════════════════════════════
# 🎨 PREMIUM DARK CSS — Original theme preserved + enhanced
# ══════════════════════════════════════════════════════════════════
st.markdown("""
<style>
...
def get_tradingview_symbol(symbol: str) -> str:
    # 🌟 ઓરિજિનલ મેપિંગને ભારતીય બજાર સોર્સ પર સેટ રાખ્યું છે
    tv_map = {
        "NIFTY"      : "NSE:NIFTY1!",
        ...
        "CRUDOIL"    : "MCX:CRUDEOIL1!",  # સ્પેલિંગ સેફ્ટી
...
if not st.session_state["authenticated"]:
    ...
    with col_mid:
        ...
        login_user = st.text_input("👤 User ID:",
            placeholder="Enter User ID", key="li_u")
        login_pass = st.text_input("🔑 Password:",
            type="password", placeholder="Default: 1234", key="li_p")
...
st.markdown(
    ...
    unsafe_allow_html=True)
st.markdown("<hr style='margin:5px 0 15px 0;border-color:#1e366a;'>",
            unsafe_allow_html=True)
...
m1.metric("📊 Today P&L (Live)",
          f"₹ {pnl:,.2f}", pnl_delta)
m2.metric("💰 Total Capital",
          f"₹ {st.session_state['total_capital']:,.0f}", "Available")
m3.metric("🎯 Open Positions",
          f"{len(st.session_state['open_positions'])} Active", "Running")
...
with col_ctrl:
    ...
    with st.expander("🔌 Broker API Configuration"):
        ...
        selected_broker = st.selectbox(
            "Broker પસંદ કરો:",
            [b.value for b in BrokerType], key="broker_sel")
    ...
    with cs2:
        sel_type = st.selectbox(
            "Type:", ["CE","PE","FUT","EQUITY (CASH)"], key="otype")
    ...
    # ── SL / TARGET ────────────────────────────────────────────────
    st.markdown("<div class='config-box' style='padding:10px;'>",
                unsafe_allow_html=True)
    ...
    with ob3:
        ...
        if st.button("🟨 WAIT", key="wait_b", use_container_width=True):
...
# ══════════════════════════════════════════════════════════════════
# LEFT PANEL — TRADINGVIEW CHART (🔴 FIXED & LOCKED FOR STREAMLIT)
# ══════════════════════════════════════════════════════════════════
with col_chart:
    st.markdown(
    ...
    # 🌟 લિંક બહાર જતી અટકાવવા અને Apple નો ચાર્ટ બ્લોક કરવા માટેનું અલ્ટીમેટ સ્ક્રિપ્ટ સેટિંગ
    chart_html = f"""
...
st.markdown("<h4 style='color:#f3f4f6;'>📋 Live Open Positions</h4>",
            unsafe_allow_html=True)
