...
def generate_pdf():
    ...
    # High Court Margins (Left/Right: 4cm, Top/Bottom: 2cm)
    ...
    title_style = ParagraphStyle('TitleStyle', parent=styles['Normal'], alignment=1, fontSize=14, leading=18)
    center_style = ParagraphStyle('CenterStyle', parent=styles['Normal'], alignment=1, fontSize=12, leading=16)
    body_style = ParagraphStyle('BodyStyle', parent=styles['Normal'], fontSize=11, leading=18)
    right_style = ParagraphStyle('RightStyle', parent=styles['Normal'], alignment=2, fontSize=11, leading=16)
    
    story = []
    
    # Header
    story.append(Paragraph(f"<b>મેહરબાન {court_name} માં</b>", title_style))
    story.append(Spacer(1, 10))
    
    if case_number:
        story.append(Paragraph(f"<b>{case_number}</b>", center_style))
        story.append(Spacer(1, 10))
        
    story.append(Paragraph(f"<b>અરજદાર / પ્રથમ પક્ષકાર:</b> {applicant_name}<br/><b>વિરુદ્ધ / અને</b><br/><b>સામાવાળા / બીજો પક્ષકાર:</b> {opposite_party}", body_style))
    story.append(Spacer(1, 15))
    
    title_clean = selected_doc.split('(')[0]
    story.append(Paragraph(f"<b>:: {title_clean} ::</b>", title_style))
    story.append(Spacer(1, 15))
    
    # Replace newlines with <br/> for ReportLab HTML handling
    formatted_body = doc_body.replace('\n', '<br/>')
    story.append(Paragraph(formatted_body, body_style))
    story.append(Spacer(1, 30))
    
    story.append(Paragraph(f"_____________________<br/>({applicant_name})<br/><b>અરજદાર / પક્ષકારની સહી</b>", right_style))
...
with col_preview:
    ...
    st.success("તમારો લીગલ દસ્તાવેજ હાઇકોર્ટના માપદંદો મુજબ તૈયાર છે.")
    ...
    with col_d2:
        st.download_button(
            label="📄 PDF (.pdf) ડાઉનલોડ કરો",
            data=pdf_data,
            file_name=f"{selected_doc.split(' ')[0]}_Legal_Document.pdf",
    ...
    ---
    ### ⚖️ કાનૂની સુવિધાઓ:
    * ✅ **Word (DOCX) અને PDF બંને ફોર્મેટ સપોર્ટ**
    * ✅ **વાંધા અરજી, બચાવ અરજી, ભાડા કરાર, બનાખત, સોગંદનામું વગેરે**
    * ✅ **A4 Size, Left/Right 4cm Margin ઓટો-સેટ**
    * ✅ **AnyRoR, Garvi અને Gujarat High Court Direct Links**
    """)
