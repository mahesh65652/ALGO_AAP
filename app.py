...
# ── WEASYPRINT GUJARATI PDF GENERATOR ─────────────────────────────────
def generate_pdf():
    body_html = doc_body.replace('\n', '<br/>')
    title_clean = selected_doc.split('(')[0]
    case_html = f"<div class='center'><b>{case_number}</b></div>" if case_number else ""
    ...
    <head>
        ...
        <style>
            ...
            body {{
                font-family: 'Noto Sans Gujarati', 'Shruti', 'Mukta Vaani', sans-serif;
                font-size: 13pt;
                line-height: 1.6;
                color: #000;
            }}
    ...
    <body>
        <div class="center">
            <h2>મેહરબાન {court_name} માં</h2>
        </div>
        {case_html}
        <div class="party-info">
            <b>અરજદાર / પ્રથમ પક્ષકાર:</b> {applicant_name}<br/>
            <b>વિરુદ્ધ / અને</b><br/>
            <b>સામાવાળા / બીજો પક્ષકાર:</b> {opposite_party}
        </div>
        <div class="title">:: {title_clean} ::</div>
        <div class="content">
            {body_html}
        </div>
        <div class="right">
...
with col_preview:
    ...
    ---
    ### ⚖️ કાનૂની સુવિધાઓ:
    * ✅ **Word (DOCX) અને PDF બંને ફોર્મેટ ૧૦૦% ગુજરાતી ફોન્ટ સપોર્ટ સાથે**
    * ✅ **A4 Size, Left/Right 4cm Margin ઓટો-સેટ**
    * ✅ **AnyRoR, Garvi અને High Court Direct Links**
    """)
