...
# ── 3CX STYLE ULTRA MODERN DARK NEON + SOFT SIDEBAR CSS ────────────
st.markdown("""
    <style>
    .stApp {
    ...
    /* Modern Soft Sidebar Styling */
    section[data-testid="stSidebar"] {
        background-color: #051024 !important;
        border-right: 1px solid #173b70;
    }
    section[data-testid="stSidebar"] div[data-testid="stButton"] > button {
        background: linear-gradient(145deg, #0d2247, #091730) !important;
        border: 1px solid #1a4580 !important;
    ...
    section[data-testid="stSidebar"] div[data-testid="stButton"] > button:hover {
        background: linear-gradient(145deg, #1890ff, #106eba) !important;
        color: #ffffff !important;
...
with st.sidebar:
    ...
    # 📧 જિલ્લા & તાલુકા સરકારી ઈમેલ સર્ચ
    with st.expander("📧 જિલ્લા / તાલુકા ઈમેલ શોધકો", expanded=True):
        sel_dist = st.selectbox("જિલ્લો પસંદ કરો:", list(GOVT_DISTRICT_EMAILS.keys()))
        if sel_dist:
            offices_dict = GOVT_DISTRICT_EMAILS[sel_dist]
            sel_office = st.selectbox("તાલુકા / કચેરી પસંદ કરો:", list(offices_dict.keys()))
