...
# ── DATABASE SETUP (SQLite with Context Manager) ─────────────────
...
# ── SIDEBAR (CLEANER & DROPDOWN ACCORDION) ────────────────────────
with st.sidebar:
...
with tab1:
    ...
    with col_input:
        ...
        with c2:
            opposite_party = st.text_input("સામાવાળાનું નામ:", "ગુજરાત રાજ્ય / સામાવાળાનું નામ")
        ...
        st.caption("📌 Margins: Left/Right 4cm, Top/Bottom 2cm | Line Spacing: 1.5")
...
with tab2:
    st.subheader("🔒 તમારા સુરક્ષિત દસ્તાવેજો અને લાઈવ એડિટિંગ")
    if not st.session_state["logged_in"]:
