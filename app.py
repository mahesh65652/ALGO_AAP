...
with hc2:
    ...
    if st.button("🌙" if dark else "☀️", key="theme_btn", help="Dark/Light Mode"):
...
# ══════════════════════════════════════════════════════════════════
# SYMBOL SELECTOR + DIRECT LIVE WIDGET CHART (NO SETTINGS REQUIRED)
# ══════════════════════════════════════════════════════════════════
...
with chart_col2:
    tf = st.selectbox("Timeframe:", ["1","3","5","15","30","60","D"], index=3, label_visibility="collapsed")

# TradingView સીધો ઓટોમેટીક ડેટા સેટઅપ
tv_theme = "dark" if dark else "light"
chart_symbol = get_tv_sym(usym)

# 100% ઓટોમેટીક ચાલે તેવો કોડ (યુઝરે કશું સેટ નથી કરવાનું)
tradingview_html = f"""
<!DOCTYPE html>
<html>
<head>
...
def _place(action):
    if not st.session_state["broker_connected"] and st.session_state["auto_trade"]:
        st.error("❌ Broker karkvo connected!"); return
...
with st.expander("🔌 Broker API & Telegram Setup"):
    ...
    with tc1:
        tg_tok = st.text_input("Bot Token:", type="password", key="tgt", value=st.session_state.get("tg_token",""))
    with tc2:
        tg_cid = st.text_input("Chat ID:", key="tgc", value=st.session_state.get("tg_chat_id",""))
