"""
app.py — RAMAVAT ALGO [v8.0 - FINAL STABLE & PREMIUM FULL-SERVICE BROKER]
========================================================================
"""
...
import brokers     # આપણું બ્રોકર લોગિન મગજ
import orders      # આપણું ઓર્ડર ફાયર મગજ

# ── PAGE CONFIG ────────────────────────────────────────────────────
st.set_page_config(
    page_title="Ramavat Algo v8.0",
    page_icon="🔱",
    layout="wide",
...
# ══════════════════════════════════════════════════════════════════
# 🎨 ULTRA FANCY PREMIUM STYLES WITH FIXED ELEMENT IDS
# ══════════════════════════════════════════════════════════════════
st.markdown("""
<style>
...
<div class="fancy-title-container">
    ...
    <div class="subtitle-terminal">⚡ PREMIUM QUANT TRADING TERMINAL • FULL-SERVICE DESK ⚡</div>
...
m4.metric("🔌 Full-Service Broker", st.session_state["active_broker"])
...
with col_ctrl:
    # 🔑 STEP 1: PREMIUM FULL-SERVICE BROKER LOGIN PANEL
    st.markdown("<h4>🔑 STEP 1: PREMIUM FULL-SERVICE BROKER AUTHENTICATION</h4>", unsafe_allow_html=True)
    st.markdown("<div class='config-box'>", unsafe_allow_html=True)
    
    broker_choice = st.selectbox("પસંદગીના પ્રીમિયમ બ્રોકર પસંદ કરો:", [
        "ICICI Direct (Breeze API)", 
        "HDFC Securities (DigiAPI)", 
        "Kotak Securities (Neo Link)", 
        "Motilal Oswal (MO API)", 
        "Sharekhan (Trade Tiger API)"
    ], key="broker_selection_box")
    
    brow1, brow2 = st.columns(2)
    with brow1:
        c_id = st.text_input("Broker Client Code / ID:", placeholder="e.g., ICICI1029", key="broker_client_id_input")
    with brow2:
        a_key = st.text_input("App Key / Access Token:", type="password", placeholder="••••••••", key="broker_api_key_input")
        
    st.markdown('<div class="login-btn-box">', unsafe_allow_html=True)
    if st.button("🔌 SECURE PREMIUM API LOGIN", use_container_width=True, key="broker_submit_login_btn"):
    ...
    with c1:
        ...
        if st.button("SEMI AUTO", use_container_width=True, key="mode_semi_auto_btn"):
    ...
    with c2:
        ...
        if st.button("FULLY AUTO", use_container_width=True, key="mode_fully_auto_btn"):
            st.session_state["algo_mode"] = "Fully Automated"
            st.toast("🔥 FULLY AUTOMATED ALGO RUNNING LIVE!")
        st.markdown('</div>', unsafe_allow_html=True)
    with c3:
        st.markdown('<div class="algo-btn-scalp">', unsafe_allow_html=True)
        if st.button("SCALPING", use_container_width=True, key="mode_scalping_btn"):
    ...
    chosen_strat = st.selectbox("Select Core Indicator Strategy Logic:", 
                                  ["Supertrend (10, 3) Pro", "RSI Reversal (14)", "MACD Crossover Cross", "Price Action Breakout"], key="core_strategy_selectbox")
    st.session_state["selected_strategy"] = chosen_strat
    
    user_symbol = st.text_input("🔍 Quick Symbol Search:", value="SBIN", key="symbol_search_input").upper().strip()
    sel_type = st.selectbox("Product Type:", ["EQUITY (CASH)", "CE", "PE", "FUT"], key="product_type_selectbox")
    u_lots = st.number_input("🔢 Quantity / Lots:", min_value=1, value=10, key="quantity_lots_number_input")
    st.markdown("</div>", unsafe_allow_html=True)
    ...
    with b1:
        ...
        if st.button("BUY", use_container_width=True, key="execution_buy_override_btn"):
    ...
    with b2:
        ...
        if st.button("SELL", use_container_width=True, key="execution_sell_override_btn"):
