...
# ── DATABASE SETUP ─────────────────────────────────────────────────
DB_FILE = "legal_drafts_secure_v9.db"
...
def save_user_draft(username, doc_type, property_type, court_name, applicant_name, opposite_party, case_number, doc_body):
    with sqlite3.connect(DB_FILE) as conn:
        c = conn.cursor()
        c.execute("""
            INSERT INTO drafts (username, doc_type, property_type, court_name, applicant_name, opposite_party, case_number, doc_body)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        """, (username, doc_type, property_type, court_name, applicant_name, opposite_party, case_number, doc_body))

def update_user_draft(draft_id, property_type, court_name, applicant_name, opposite_party, case_number, doc_body):
    with sqlite3.connect(DB_FILE) as conn:
        c = conn.cursor()
        c.execute("""
            UPDATE drafts
            SET property_type = ?, court_name = ?, applicant_name = ?, opposite_party = ?, case_number = ?, doc_body = ?
            WHERE id = ?
        """, (property_type, court_name, applicant_name, opposite_party, case_number, draft_id))
...
def get_user_drafts(username, search_query="", is_admin=False):
    with sqlite3.connect(DB_FILE) as conn:
        ...
        if is_admin:
            c.execute("""
                SELECT id, username, doc_type, property_type, applicant_name, created_at, court_name, opposite_party, case_number, doc_body
                FROM drafts
...
સાહેબશ્રી,
આ અદાલતને સદર વિષયવસ્તુ કે આર્થિક अधिकारક્ષેત્ર (Jurisdiction) ન હોવાથી, વાદીને યોગ્ય કોર્ટમાં દાવો રજૂ કરવા માટે પ્લેઈન્ટ પરત આપવા નમ્ર વિનંતી.
...
# ── WORD GENERATOR ─────────────────────────────────────────────────
def generate_docx(court_name, selected_doc, property_type, case_number, applicant_name, opposite_party, doc_body):
    doc = Document()
    ...
    if property_type and property_type != "કોઈપણ નહિ (N/A)":
        p_prop = doc.add_paragraph()
        p_prop.alignment = WD_ALIGN_PARAGRAPH.CENTER
        run_prop = p_prop.add_run(f"પ્રોપર્ટીનો પ્રકાર: {property_type}\n")
...
with tab1:
    ...
    with col_input:
        ...
        # ── PROPERTY TYPE SELECTOR (ADDED FROM SCREENSHOT) ──
        property_options = [
            "કોઈપણ નહિ (N/A)",
            "Residential Flat / રહેણાંક ફ્લેટ",
            "Residential House / રહેણાંક મકાન",
            "Commercial Property / કોમર્શિયલ પ્રોપર્ટી",
            "Land Plot / પ્લોટ",
            "Agricultural Land / ખેતીની જમીન",
            "Industrial Property / ઔદ્યોગિક પ્રોપર્ટી",
            "Other / અન્ય"
        ]
        property_type = st.selectbox("🏘️ Property Type / પ્રોપર્ટીનો પ્રકાર:", property_options)
        ...
        if st.session_state["logged_in"]:
            if st.button("💾 એકાઉન્ટમાં સેવ કરો", use_container_width=True, type="primary"):
                save_user_draft(
                    st.session_state["username"], selected_doc, property_type, court_name,
    ...
    with col_preview:
        ...
        docx_data = generate_docx(court_name, selected_doc, property_type, case_number, applicant_name, opposite_party, doc_body)
        ...
        prop_str = f"<div class='center'><b>પ્રોપર્ટીનો પ્રકાર:</b> {property_type}</div><br/>" if property_type != "કોઈપણ નહિ (N/A)" else ""
        ...
        <body>
            ...
            <script>
                function printDoc() {{
                    ...
                    printWindow.document.write('{prop_str}');
...
with tab2:
    ...
    if not st.session_state["logged_in"]:
        ...
        if not drafts:
            ...
            for d in drafts:
                draft_id, user_owner, d_type, p_type, app_name, c_time, c_name, opp_p, c_num, d_body = d
                with st.expander(f"📜 {d_type} | પ્રકાર: {p_type or 'N/A'} | અરજદાર: {app_name} | ({c_time})"):
                    st.write(f"**યુઝર:** {user_owner}")
                    edit_p_type = st.selectbox("પ્રોપર્ટીનો પ્રકાર:", property_options, index=property_options.index(p_type) if p_type in property_options else 0, key=f"pt_{draft_id}")
                    ...
                    with col_save_btn:
                        if st.button("✏️ સુધારા સેવ કરો", key=f"up_{draft_id}", type="primary"):
                            update_user_draft(draft_id, edit_p_type, edit_c_name, edit_app_name, edit_opp_p, edit_c_num, edit_d_body)
                            st.success("સુધારો સેવ થઈ ગયો!")
                            st.rerun()

                    with col_dl_btn:
                        saved_docx = generate_docx(edit_c_name, d_type, edit_p_type, edit_c_num, edit_app_name, edit_opp_p, edit_d_body)
                        st.download_button(
                            label="📥 Word ડાઉનલોડ",
                            data=saved_docx,
