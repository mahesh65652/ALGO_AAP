...
with tab1:
    ...
    with col_input:
        ...
        # ── 50 અરજીઓ માટે ડ્રોપડાઉન (SELECTBOX) ──────────────────
        selected_app = st.selectbox(
            "અરજીનો પ્રકાર પસંદ કરો:",
            options=app_keys_list,
            index=0
        )
        ...
        case_number = st.text_input("અરજી ક્રમાંક / ઓળખ નંબર (જો હોય તો):", "અરજી નંબર: ૨૦૨૬/GP/૧૦૧")
        
        app_clean_title = selected_app.split('. ')[1] if '. ' in selected_app else selected_app
        default_body = f"""સવિનય જણાવવાનું કે હું અરજદાર શ્રી {applicant_name}, રહેવાસી {village_info} નો કાયમી રહવાસી છું.

ઉપરોક્ત દર્શાવેલ વિષય અન્વયે જણાવવાનું કે મારે "{app_clean_title}" અંગેની કામગીરી જરૂરિયાત હોય, આ અંગેની સત્તાવાર અરજી આપ સાહેબશ્રી સમક્ષ રજૂ કરું છું.
...
૨. સંબંધિત પુરાવાઓની નકલ."""
    ...
    with col_preview:
        ...
        st.download_button(
            ...
            data=docx_data,
            file_name=f"{app_clean_title}_Application.docx",
