...
dynamic_pnl = 2695 + (current_time_sec % 10 * 5)
...
# --- ૫. લેઆઉટ સ્પ્લિટ: ચાર્ટ (ડાબે) અને કંટ્રોલ પેનલ (જમણે) ---
col_chart, col_control = st.columns([1.1, 1])

# ⚙️ જમણી બાજુના ઇનપુટ્સ
with col_control:
    # 🔐 બ્રોકર API સેટિંગ્સ
    ...
    # 🔍 ઓર્ડર કંટ્રોલ અને સ્માર્ટ સર્ચ બોક્સ (નવો સુધારો!)
    st.markdown("<div class='config-box'>", unsafe_allow_html=True)
    st.markdown("<p style='color:#e2e8f0; font-size:14px; font-weight:bold; margin:0 0 5px 0;'>🔍 ઇન્ડેક્સ અથવા સ્ટોક સર્ચ કરો:</p>", unsafe_allow_html=True)
    
    col_sym_search, col_opt = st.columns(2)
    with col_sym_search:
        # અહીં યુઝર કાંઈ પણ ટાઈપ કરી શકે છે: RELIANCE, SBIN, NIFTY, CRUDEOIL
        user_symbol = st.text_input("સિમ્બોલ નામ લખો (દા.ત. NIFTY, RELIANCE, CRUDEOIL):", value="NIFTY").strip().upper()
    with col_opt:
        selected_option = st.selectbox("ટાઈપ પસંદ કરો:", ["CE", "PE", "FUT", "EQUITY (CASH)"])

    selected_strike = st.text_input("સ્ટ્રાઈક પ્રાઈઝ (જો ઓપ્શન હોય તો):", value="22000")
    
    # ઇન્ટેલિજન્ટ ઓટો લોટ કેલ્ક્યુલેટર (ઇન્ડેક્સ, કોમોડિટી અને સ્ટોક્સ બધા માટે)
    lot_sizes = {
        "NIFTY": 25, "BANKNIFTY": 15, "FINNIFTY": 25,
        "CRUDEOIL": 100, "NATURALGAS": 1250, "GOLD": 100, "SILVER": 30
    }
    # જો લિસ્ટની બહારનો કોઈ સ્ટોક હોય (જેમ કે RELIANCE, SBIN), તો લોટ સાઇઝ ઓટોમેટીક ૧ ગણાશે (એટલે કે શેર વાઈઝ ક્વોન્ટિટી)
    current_lot_size = lot_sizes.get(user_symbol, 1)
    
    col_l, col_q = st.columns(2)
    u_lots = col_l.number_input("🔢 લોટ / ક્વોન્ટિટી ગુણક:", min_value=1, value=1)
    u_qty = u_lots * current_lot_size
    
    qty_label = "કુલ શેર ક્વોન્ટિટી (Shares):" if current_lot_size == 1 else "કુલ ડેરિવેટિવ ક્વોન્ટિટી (Auto):"
    col_q.text_input(qty_label, value=f"{u_qty} Qty", disabled=True)
    st.markdown("</div>", unsafe_allow_html=True)
    ...
    with b1:
        ...
        if st.button("🟩 BUY"):
            if st.session_state["broker_connected"]:
                st.toast(f"🛒 BUY ORDER FIRED! {user_symbol} | Qty: {u_qty}", icon="✅")
            else:
    ...
    with b2:
        ...
        if st.button("🟥 SELL"):
            if st.session_state["broker_connected"]:
                st.toast(f"📉 SELL ORDER FIRED! {user_symbol} | Qty: {u_qty}", icon="🚨")
            else:
                st.error("❌ પેલા બ્રોકર API કનેક્ટ કરો સાહેબ!")
        st.markdown('</div>', unsafe_allow_html=True)
...
# 📈 ડાબી બાજુ: અસલી લાઇવ ડાયનેમિક સર્ચ બેઝ્ડ ચાર્ટ
with col_chart:
    st.markdown(f"<h4 style='color:#f3f4f6;'>📈 લાઇવ ચાર્ટ: {user_symbol}</h4>", unsafe_allow_html=True)
    
    # સ્માર્ટ ચાર્ડ મેપિંગ: જો સ્ટોક ઇન્ડિયન હોય તો આપોઆપ NSE નો ગ્રાફ પકડશે!
    symbol_map = {
        "CRUDEOIL": "FX:USOIL",
    ...
    # જો ટાઈપ કરેલો સિમ્બોલ ઉપરના લિસ્ટમાં ન હોય, તો એને ઓટોમેટીક ઇન્ડિયન સ્ટોક માનીને "NSE:સ્ટોક_નામ" સેટ કરી દેશે (દા.ત. NSE:RELIANCE)
    tv_symbol = symbol_map.get(user_symbol, f"NSE:{user_symbol}")
...
script_name = f"{user_symbol} {selected_strike} {selected_option}" if "CE" in selected_option or "PE" in selected_option else f"{user_symbol} ({selected_option})"

my_trades = {
    "ટ્રેડ આઈડી (ID)": ["#RM-2001"],
    "સિમ્બોલ (Script)": [script_name],
    "કુલ ક્વોન્ટિટી": [f"{u_qty} Pcs/Shares" if current_lot_size == 1 else f"{u_qty} ({u_lots} Lots)"],
