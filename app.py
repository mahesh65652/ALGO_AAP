"""
app.py — RAMAVAT ALGO ELITE  [v3.1]
=====================================
Mobile-First | Centered Chart with Full Border
...
@keyframes ppulse {{
  ...
  100%{{box-shadow:0 0 0 0 rgba(220,38,38,.7)}}
}}
...
/* 📦 FULL BORDER CHART CONTAINER */
.chart-container {{
  background:var(--card); 
  border: 3px solid var(--border) !important; /* ફૂલ મજબૂત બોર્ડર */
  border-radius:16px; 
  padding: 6px;
  margin: 12px 0;
  box-shadow: 0 0 20px rgba(59, 130, 246, 0.15); /* આકર્ષક ગ્લો */
  overflow:hidden;
...
def trigger_order(symbol, action, qty, exchange="NFO", sl=0.0, target=0.0):
    ...
    add_log(f"{action} [{mode}]", f"{qty}x {symbol} SL:{sl} TGT:{target}")
...
if not st.session_state["authenticated"]:
    ...
    lp = st.text_input("🔑 Password", type="password", placeholder="Password", key="lp", label_visibility="collapsed")
...
if not risk_ok:
    st.markdown(f"<div class='risk-box'><b style='color:#fca5a5;'>⚠️ {risk_msg}</b></div>", unsafe_allow_html=True)
...
# ── 2. EXPANDER SETUPS (TOP POSITION) ──────────────────────────────
with st.expander("🔌 Broker API & Telegram Setup"):
    ...
    if st.button("🔌 Connect Broker", use_container_width=True, key="conn"):
        if ucid and uapi:
            st.session_state.update({"broker_connected":True,"connected_broker":sel_b})
            add_log("BROKER_CONNECTED", sel_b); st.rerun()
    st.markdown("</div>", unsafe_allow_html=True)

    st.markdown('<div class="sec-lbl">📱 TELEGRAM ALERTS</div>', unsafe_allow_html=True)
    st.markdown("<div class='cfg'>", unsafe_allow_html=True)
    tc1,tc2 = st.columns(2)
    with tc1: tg_tok = st.text_input("Bot Token:", type="password", key="tgt", value=st.session_state.get("tg_token",""))
    with tc2: tg_cid = st.text_input("Chat ID:", key="tgc", value=st.session_state.get("tg_chat_id",""))
    if st.button("📱 Test Telegram", use_container_width=True, key="tg_test"):
        st.session_state["tg_token"], st.session_state["tg_chat_id"] = tg_tok, tg_cid
        send_telegram("✅ Ramavat Algo Elite Connection OK!")
    st.markdown("</div>", unsafe_allow_html=True)

with st.expander("🛡️ Risk Management"):
    st.markdown("<div class='cfg'>", unsafe_allow_html=True)
    rr1,rr2 = st.columns(2)
    with rr1: st.session_state["max_daily_loss"] = st.number_input("Max Daily Loss (₹):", value=int(st.session_state["max_daily_loss"]))
    with rr2: st.session_state["daily_target"] = st.number_input("Daily Target (₹):", value=int(st.session_state["daily_target"]))
    st.session_state["trail_active"] = st.toggle("🔄 Auto Trail SL", value=st.session_state["trail_active"])
    st.markdown("</div>", unsafe_allow_html=True)


# ── 3. CENTER: FULL BORDER LIVE CHART (બિલકુલ વચ્ચે) ──────────────────
st.markdown('<div class="sec-lbl">📈 CENTER LIVE CHART (FULL BORDER)</div>', unsafe_allow_html=True)

chart_col1, chart_col2 = st.columns([3, 1])
with chart_col1:
    sym_opts = ["NIFTY","BANKNIFTY","SENSEX","FINNIFTY","CRUDEOIL","NATURALGAS","GOLD","SILVER"]
    usym = st.selectbox("Symbol:", sym_opts, index=sym_opts.index(st.session_state.get("chart_symbol","NIFTY")), key="chart_sym_sel", label_visibility="collapsed")
    st.session_state["chart_symbol"] = usym
with chart_col2:
    tf = st.selectbox("Timeframe:", ["1","3","5","15","30","60","D"], index=2, label_visibility="collapsed")

# TradingView HTML Setup
...
<head>
    ...
    <style>
        ...
        #tv-chart-container {{ height: 540px; width: 100%; }}
    </style>
</head>
<body>
...
# ફૂલ બોર્ડર વાળું કન્ટેનર બોક્સ
st.markdown(f'<div class="chart-container">', unsafe_allow_html=True)
st.components.v1.html(tradingview_html, height=540, scrolling=False)
st.markdown('</div>', unsafe_allow_html=True)


# ── 4. BOTTOM: ORDER CONTROLS & POSITION TABLE ─────────────────────
st.markdown('<div class="sec-lbl">⚙️ ORDER CONTROL CENTER</div>', unsafe_allow_html=True)
...
with lc1: ulots = st.number_input("Lots:", min_value=1, value=1, key="lots")
with lc2: st.text_input("Total Qty:", value=f"{ulots * ls} ({ls}/lot)", disabled=True)

sc1,sc2 = st.columns(2)
with sc1: sl_pts  = st.number_input("🚨 SL (Pts):",  value=30)
with sc2: tgt_pts = st.number_input("🎯 TGT (Pts):", value=60)
st.markdown("</div>", unsafe_allow_html=True)

st.session_state["auto_trade"] = st.toggle(f"{'🟢 LIVE TRADE MODE' if st.session_state['auto_trade'] else '🟡 DRY RUN MODE'}", value=st.session_state["auto_trade"])
...
def _place(action):
    if not st.session_state["broker_connected"] and st.session_state["auto_trade"]:
        st.error("❌ Broker કનેક્ટ કરો પ્રથમ!"); return
    if not risk_ok: st.error(f"⛔ {risk_msg}"); return
    res = trigger_order(script, action, ulots*ls, exchange="NFO" if otype in ["CE","PE","FUT"] else "NSE", sl=sl_pts, target=tgt_pts)
    st.session_state["open_positions"].append({
        "id":f"#RM-{len(st.session_state['open_positions'])+2001}","script":script,"qty":ulots*ls,
        "action":action,"sl":sl_pts,"tgt":tgt_pts,"status":"⌛ Active","mode":mode_str
    })
    st.toast(f"✅ {action} Order Place થઈ ગયો!")
...
with b3:
    ...
    if st.button("🟨 REFRESH P&L", key="ref", use_container_width=True):
        st.session_state["dynamic_pnl"] += random.uniform(-100,200); st.rerun()
    st.markdown("</div>", unsafe_allow_html=True)
...
if st.button("💥 EMERGENCY CLOSE ALL POSITIONS", key="panic", use_container_width=True):
    ...
    add_log("EMERGENCY CLOSE", "All positions squared off")
    st.success("🚨 બધી પોઝિશન્સ ક્લોઝ થઈ ગઈ છે!"); time.sleep(0.5); st.rerun()
st.markdown("</div>", unsafe_allow_html=True)

# 📋 OPEN POSITIONS TABLE
st.markdown('<div class="sec-lbl">📋 LIVE OPEN POSITIONS</div>', unsafe_allow_html=True)
if st.session_state["open_positions"]:
    df = pd.DataFrame(st.session_state["open_positions"])
    st.dataframe(df, use_container_width=True, hide_index=True)
    if st.button("🗑️ Clear List", use_container_width=True):
        st.session_state["open_positions"] = []; st.rerun()
else:
    st.info("ℹ️ કોઈ Open Position નથી. ઓર્ડર પ્લેસ કરો!")
...
<div style='text-align:center;color:{MUTED};font-size:.65rem;padding:14px 0;'>
  🔱 Ramavat Algo Elite v3.1 &nbsp;|&nbsp; NSE • MCX • Equity Terminal<br>
