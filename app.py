"""
app.py — RAMAVAT ALGO ELITE [v4.6]
====================================
100% Stable Multi-Client API & TOTP Live Execution Engine
Fixed NameError on TradingView Frame String Formatting
"""
...
st.set_page_config(
    page_title="Ramavat Algo Elite v4.6",
    page_icon="🔱",
    layout="wide",
...
if "telegram_alerts_history" not in st.session_state:
    st.session_state["telegram_alerts_history"] = [
        {"time": "12:04:00", "msg": "📊 MCX Intraday Classic v3.0 | 5 Min | ATR SL/TGT: ON ✅"},
    ]
...
# MAIN HEADER
st.markdown("<h1>🔱 RAMAVAT MULTI-CLIENT LIVE TERMINAL v4.6</h1>", unsafe_allow_html=True)
...
with col_left:
    ...
    # --- 🛠️ FIX: String interpolation error resolved completely ---
    tv_url = f"https://s.tradingview.com/widgetembed/?symbol=MCX:{symbol_input}1!&interval=5&theme={tv_theme}&style=1&timezone=Asia%2FKolkata&locale=en"
    
    st.components.v1.html(f"""
    <div style="height:350px; border-radius:12px; overflow:hidden; border:1px solid #1e366a;">
        <iframe src="{tv_url}" width="100%" height="100%" frameborder="0" allowtransparency="true" scrolling="no"></iframe>
    </div>""", height=360)
...
with col_right:
    # 🔐 CLIENT API KEY & TOTP MANAGEMENT
    st.markdown("<div class='config-box'>", unsafe_allow_html=True)
    ...
    c_id = st.text_input("👤 Client ID:", value=client_data["client_id"])
    ...
    t_secret = st.text_input("🛡️ TOTP Google Auth Secret Key:", value=client_data["totp_secret"], type="password")
    ...
    if st.button(f"💾 LOCK {selected_client.upper()} CREDENTIALS", use_container_width=True):
        ...
        st.success(f"✅ {selected_client} નો ડેટા સિક્યોરલી સેવ થઈ ગયો!")
        time.sleep(0.5); st.rerun()
    st.markdown("</div>", unsafe_allow_html=True)
...
if st.session_state["open_positions"]:
    ...
    st.info("📭 હાલમાં કોઈ લાઈવ ઓર્ડર ફાયર થયો નથી.")
