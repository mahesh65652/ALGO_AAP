...
st.markdown("""
    ...
    div[data-baseweb="select"] > div { max-height: 300px !important; overflow-y: auto !important; }
    </style>
""", unsafe_allow_html=True)

# ── DATABASE SETUP ─────────────────────────────────────────────────
DB_FILE = "gram_panchayat_legal_v2.db"
...
# ── 50 APPLICATIONS CATEGORIZED FOR SMOOTH DROPDOWN ────────────────
PANCHAYAT_CATEGORIES = {
    "તલાટી-કમ-મંત્રી સંબંધિત અરજીઓ": {
    ...
    "સરપંચ / ગ્રામ પંચાયત સામાન્ય અરજીઓ": {
    ...
    "TDO / મામલતદાર / RTI / અન્ય અધિકારીઓ": {
...
with tab1:
    ...
    with col_input:
        st.subheader("📝 અરજી અને વિગતો પસંદ કરો")
        
        # 1. કેટેગરી ફિલ્ટર (જેથી Dropdown સ્ક્રોલિંગ પ્રોબ્લેમ દૂર થાય)
        category = st.selectbox("૧. કચેરી / કેટેગરી પસંદ કરો:", list(PANCHAYAT_CATEGORIES.keys()))
        
        # 2. પસંદ કરેલી કેટેગરી મુજબ અરજીઓ
        app_dict = PANCHAYAT_CATEGORIES[category]
        selected_app = st.selectbox("૨. અરજીનો પ્રકાર પસંદ કરો:", list(app_dict.keys()))
        
        suggested_auth = app_dict[selected_app]
...
૨. સંબંધિત પુરાવાઓની નકલ."""
        ...
        doc_body = st.text_area("મુખ્ય લખાણ (એડિટ કરી શકાય તેવું):", value=default_body, height=250)
